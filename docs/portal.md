...
The **Ylang Portal** is the server-rendered admin UI (`/console/*`) for a
local or LAN HTTP deployment. It is the operator control plane: health,
routing, templates, facts, parameters, proposals, privacy, and ops.

Open it in a normal browser. After login you do **not** need a Bearer header
extension.

Published screenshots below were captured from a live instance on this host
(`scripts/capture-console-screenshots.py`). PNGs live in
[`docs/images/console/`](images/console/) and ship with GitHub Pages.

Regenerate:

```bash
export YLANG_AUTH_TOKEN=…   # or let the script read /srv/ylang/ylang.env
python scripts/capture-console-screenshots.py
```

Related: [configuration.md](configuration.md) (every env/runtime key),
[deployment.md](deployment.md) (systemd), [models.md](models.md) (routing).
...
| It is | It is not |
|-------|-----------|
| Operator UI over the **same SQLite** the MCP server and gateway use | A hosted SaaS console |
| Session-cookie login with `YLANG_AUTH_TOKEN` | OAuth / per-user accounts |
| Hot-reload for runtime keys (Parameters) | A place to paste API keys (those stay in `ylang.env`) |
| Propose-only Apply for optimizer suggestions | Auto-apply of routing or templates |

HTTP transport (`YLANG_TRANSPORT=http`) is required. Stdio MCP has no Portal.
...
Primary nav is grouped in `console/layout.py`. Gated and less-used items live
under **Advanced**. Experiments / Feedback appear in primary nav only when
their flags are on.

| Nav | Route | Group | Role |
|-----|-------|-------|------|
| Today | `/console/today` | Core | Last 24h requests, spend, success/fail, latency, local vs cloud |
| Quality | `/console/quality` | Core | 7d improver funnel + weak-accept templates |
| Routing | `/console/routing` | Core | Model mix, fallbacks, explainable one-liners |
| Control | `/console/control` | Core | **Operator Hub** — Optimize wizard, KPIs, proposals, inventory |
| Overview | `/console` | Core | Health, suggestions, improver sandbox |
...
| Privacy | `/console/privacy` | Optimize | Capture level, retention, observation limits |
| Experiments | `/console/experiments` | Gated | A/B improver variants (`experiments` on) |
| Feedback | `/console/feedback` | Gated | Edit-distance events (`edit_feedback` on) |
| Parameters | `/console/settings` | System | Hot-reload runtime config + presets |
| Data | `/console/data` | Advanced | SQLite browser + safe mutators |
| Advisor | `/console/advisor` | Advanced | LLM Q&A over live analytics |
| Ops | `/console/ops` | Advanced | Backup / export / import / restore |
| Setup | `/console/setup` | Primary until done | Onboarding checklist |
| Health | `/console/health` | Linked | Provider checks + hook log |

Deep links still work when a gated page is hidden. Empty states tell you which
flag to enable.
...
Sign in with `YLANG_AUTH_TOKEN`. Optional `?next=` redirects after success
(default `/console`). Failed attempts stay on this page with an error.
...
Unauthenticated `/console/*` (except `/console/login` and `/console/static/*`)
redirects here. `/health` stays public.

---

## Today

**Route:** `GET /console/today`

Last **24 hours**: request count, spend vs budget cap, success/failure, p50/p95
latency, local vs cloud split. All numbers come from SQLite usage aggregates
(`usage/operator_metrics.py`).

![Today hub](images/console/today.png)

Use this page to answer: *is anything on fire right now?*

---

## Quality

**Route:** `GET /console/quality`

Seven-day improver funnel (fired, accept %, validation %) plus templates whose
accept rate is below 50% in that window.

![Quality hub](images/console/quality.png)

Signals are **user + heuristic** (accept / validation), not “silence as
success”. Drill into [Improver](#improver) or [Templates](#templates) for the
full table.
...
**Route:** `GET /console/routing`

Seven-day model mix, fallback event count, provider failure **classes**, and
explainable one-liners from `routing_reason_json`
(`Selected <model> because <resolution_reason>`).

![Routing hub](images/console/routing.png)

Typical reasons: `activity_default`, `operator_override`, `compatibility_alias`,
`cost_tiebreak`, `provider_unavailable`, `budget_fallback`. Full taxonomy:
[models.md](models.md#resolution-reasons).
...
![Operator Hub](images/console/control.png)
...
Each completion stores `routing_reason_json` with `resolution_reason`,
`requested_alias` / `alias_source`, and `semantic_route`. Changing Parameters
`models_*` lists vs the env baseline records `operator_override` rather than
`activity_default`.
...
**Route:** `GET /console/privacy`

Capture policy for traces stored in SQLite.

![Privacy hub](images/console/privacy.png)

| Card | Meaning |
|------|---------|
| Capture level | Effective `capture_level` (`off` / `minimal` / `redacted` / `full_local`) |
| Retention days | Default window used by `ylang purge-traces` |
| Sensitive traces | Heuristic count of rows that still hold redacted/full bodies |

The page also lists **what Ylang cannot observe** (host agent chain-of-thought, tools outside Ylang, satisfaction without a user signal, cross-process causality without `parent_trace_id`).

Change capture from Parameters (`capture_level`) or `YLANG_CAPTURE_LEVEL`. Details: [configuration.md](configuration.md#trace-privacy).
...
- `models_code`, `models_search`, `models_reason`, `models_improve`, `models_other` — vs env baseline, traces use `resolution_reason=operator_override`
...
1. **Login** → **Today** (24h pulse) → **Control** (7d KPIs + Optimize wizard).
2. **Routing** if spend or model mix looks wrong; fix lists on **Parameters**.
3. **Quality** / **Improver** if accept rate drops; quarantine toxic templates.
4. **Privacy** before raising `capture_level`.
5. **Ops** backup before restore or bulk archive.

Env-only secrets stay in `ylang.env`. Never paste API keys into Parameters.

Full key list: [configuration.md](configuration.md). Publish this site: [publishing.md](publishing.md).
