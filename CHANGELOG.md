...
- **Cursor gateway limits documented** — Ylang stays LAN-only; Cursor Agent chat routes through Cursor's cloud, which rejects private addresses (`Access to private networks is forbidden`), so gateway routing for Agent chat is not possible without a public origin. A malformed `Override OpenAI Base URL` (Ollama's `:11434`, missing `/v1`, trailing comma) produces the misleading *User Provided API Key Rate Limit Exceeded* instead. Use MCP/hooks for Cursor and `/v1/*` for LAN-local clients.
