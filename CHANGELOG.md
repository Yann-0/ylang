...
- **Cursor gateway reachability** — Cursor's cloud rejects LAN targets (`Access to private networks is forbidden`), so `deploy/apache/install-ylang-path-proxy.sh` publishes the gateway at `https://<existing-host>/ylang/v1` with no new DNS record (`/console` and `/mcp` stay private); `verify-cursor-gateway.sh` now takes `BASE_URL`
- **Cursor Base URL troubleshooting** — document that a malformed `Override OpenAI Base URL` (Ollama's `:11434`, missing `/v1`, trailing comma) makes Cursor fall back to api.openai.com and report *User Provided API Key Rate Limit Exceeded*
