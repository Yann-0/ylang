...
- **Cursor Base URL troubleshooting** — document that a malformed `Override OpenAI Base URL` (Ollama's `:11434`, missing `/v1`, trailing comma) makes Cursor fall back to api.openai.com and report *User Provided API Key Rate Limit Exceeded*; the public `ylang.stelliane.dev` vhost needs a DNS A record you create yourself
